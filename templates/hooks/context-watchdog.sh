...
if [ "$COUNT" -eq 30 ]; then
    notify "CONTEXT_WATCHDOG [YELLOW]: 30 tool calls en esta sesión. Te acercas al límite de la zona inteligente (~40% del contexto). Si la tarea actual es compleja, considera usar /compact para preservar el contexto esencial, o finaliza la tarea actual antes de empezar otra."
elif [ "$COUNT" -eq 50 ]; then
    notify "CONTEXT_WATCHDOG [ORANGE]: 50 tool calls. Probablemente estás más allá de la zona inteligente. Señales a vigilar: Claude olvida decisiones previas, repite errores corregidos, o ignora instrucciones del CLAUDE.md. Recomendación: /compact con instrucciones de qué preservar, o /clear + reformular con lo aprendido en esta sesión."
elif [ "$COUNT" -eq 80 ]; then
    notify "CONTEXT_WATCHDOG [RED]: 80 tool calls. Alto riesgo de degradación. La compactación automática puede haber eliminado contexto crítico. RECOMENDACIÓN: /clear y empezar sesión fresca. Antes de /clear: documenta el estado actual (qué se hizo, qué queda, decisiones tomadas) para poder retomar sin fricción."
elif [ "$((COUNT % 20))" -eq 0 ] && [ "$COUNT" -gt 80 ]; then
    notify "CONTEXT_WATCHDOG [RED]: $COUNT tool calls. Sesión muy larga. Calidad de output probablemente degradada. Usa /clear."
fi
